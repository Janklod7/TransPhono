...
class EngLang(Language):
    ...
    def parse_new_features(self, save=False):
        ...
        for d in response:
            ...
            for p in phonms:
                if p != 'AH0':
                    ...
                    p = p.replace("1", "")
                    p = p.replace("2", "")
                if p in self.features.replace:
                    new_phonms.extend(self.features.replace[p])
                else:
                    new_phonms.append(p)
            phonms = new_phonms
            if len(phonms) > int(self.config['maximum_word_length']):
        ...
        pa = self.root + "datasets/Languages/English"
        if save:
            with open(f"{pa}/phonemes_{self.config['maximum_word_length']}f.json", 'w') as fp:
                json.dump(self.phonemes, fp)
            with open(f"{pa}/data_{self.config['maximum_word_length']}f.json", 'w') as fp:
                json.dump(self.data, fp)
            with open(f"{pa}/index_to_phon_{self.config['maximum_word_length']}f.json", 'w') as fp:
                json.dump(self.index_to_phon, fp)
        ...
        # print(f"Got {len(self.data) - len(self.plurals)} words")
        print(f"Got {len(self.data)} words")
        print(f"Got {len(self.phonemes)} phonemes")
        # print(f"Got {len(self.plurals)} plurals")
        print(f"Word maximum length is {maxlen}")
        print(f"{toolong} Words were too long")
